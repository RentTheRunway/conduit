...
class AMQPAsyncQueueConsumerTest {
    ...
    void testRespondMultipleAcknowledgeAll() throws Exception {
        ...
        final AMQPAsyncConsumerCallback callback =
                new AMQPAsyncConsumerCallback() {
                    ...
                    @Override
                    public void handle(
                            final AMQPMessageBundle messageBundle, final AsyncResponse response) {
        ...
        final AMQPAsyncQueueConsumer consumer =
        ...
        consumer.handleDelivery(consumerTag, envelope, properties, "world".getBytes(UTF_8));
    ...
    void testRespondMultipleRejectAndDiscardAll() throws Exception {
        ...
        final String consumerTag = "foo";
        final Envelope envelope1 = new Envelope(0, false, "exchange", "key");
        final Envelope envelope2 = new Envelope(1, false, "exchange", "key");
        final AMQP.BasicProperties properties = new AMQP.BasicProperties();

        consumer.handleDelivery(consumerTag, envelope1, properties, "hello".getBytes(UTF_8));
        consumer.handleDelivery(consumerTag, envelope2, properties, "world".getBytes(UTF_8));
    ...
    void testRespondSingleRejectAndDiscard() throws Exception {
        ...
        consumer.respondSingle(
                new AMQPMessageBundle(consumerTag, envelope1, properties, "hello".getBytes(UTF_8)),
    ...
    void testRespondMultipleRejectAndRequeueAll() throws Exception {
        ...
        final ArgumentCaptor<AMQP.BasicProperties> captor =
        ...
        reset(channel);
        consumer.handleDelivery(consumerTag, envelope1, captor.getValue(), "hello".getBytes(UTF_8));
        consumer.handleDelivery(consumerTag, envelope2, captor.getValue(), "world".getBytes(UTF_8));
        verify(channel, times(1)).basicNack(eq(1L), eq(true), eq(false));
        verify(channel, times(2))
                .basicPublish(eq("exchange"), eq("key"), captor.capture(), any(byte[].class));
    ...
    void testHandleDeliveryMixedResponses() throws Exception {
        ...
        final Envelope envelope1 = new Envelope(0, false, "exchange", "key");
        final Envelope envelope2 = new Envelope(1, false, "exchange", "key");
        final Envelope envelope3 = new Envelope(2, false, "exchange", "key");
        final Envelope envelope4 = new Envelope(3, false, "exchange", "key");
        final Envelope envelope5 = new Envelope(4, false, "exchange", "key");
        final AMQP.BasicProperties properties =
                new AMQP.BasicProperties().builder().headers(new HashMap<>()).build();

        final byte[] ack = "ack".getBytes(UTF_8);
        final byte[] discard = "discard".getBytes(UTF_8);
        final byte[] requeue = "requeue".getBytes(UTF_8);
    ...
    void testHandleDeliveryMixedRetryValues() throws Exception {
        ...
        doAnswer(
                        invocationOnMock -> {
                            ...
                            final String routingKey = invocationOnMock.getArguments()[1].toString();
        ...
        for (int i = 0; i < 6; i++) {
            final Envelope envelope = new Envelope(i, false, "exchange", "key");
            ...
            consumer.handleDelivery(
                    "foo", envelope, properties, Integer.valueOf(i).toString().getBytes(UTF_8));
        }

        // 3 of our messages should have been sent to poison queue, while 3 should have been retried
        final Map<Long, String> expectedRoutingKeys = new HashMap<>();
        ...
        final List<Long> expectedDeliveryTags = new ArrayList<>();
    ...
    void testShutdownHandlerInvocation() {
        final AMQPAsyncConsumerCallback callback = mock(AMQPAsyncConsumerCallback.class);
        final Channel channel = mock(Channel.class);
        final AMQPAsyncQueueConsumer consumer =
                spy(new AMQPAsyncQueueConsumer(channel, callback, 1, "", true));
